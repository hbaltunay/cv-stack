...
class CacheService:
    ...
    def client(self) -> redis.Redis:
        ...
        Raises:
            CacheServiceException: If the client is accessed before
                                   initialization.
        """
        if self._client is None:
            err_msg = "Redis client is not initialized. Call start() first."
            raise CacheServiceException(detail=f"Cache Error: {err_msg}")
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            logger.error(
                "Redis connection failed", extra={"params": {"error": str(err)}}
            )
            self._client = None
    ...
    async def get(self, key: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.error(
                "Cache get error", extra={"params": {"error": str(err)}}
            )
            raise CacheServiceException(detail=f"Cache Error: {err}") from err
    ...
    async def set(self, key: str, value: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(
                "Cache SET error", extra={"params": {"error": str(err)}}
            )
            raise CacheServiceException(detail=f"Cache Error: {err}") from err
